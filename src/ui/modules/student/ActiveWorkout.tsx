...
const ActiveWorkout: React.FC = () => {
  ...
  const [swappingExerciseId, setSwappingExerciseId] = useState<string | null>(
    null,
  );
  ...
  const progress = Math.round(
    (completedExercises.length / user.currentWorkout.exercises.length) * 100,
  );
    ...
    <div className="min-h-screen bg-background-dark pb-24 relative">
      ...
      <div className="sticky top-0 z-10 bg-background-dark/95 backdrop-blur-md border-b border-white/10 p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="font-lexend text-white font-bold tracking-wide">
            TREINO ATIVO
          </span>
          <div className="w-6" />
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-sparta-gold">
              {user.currentWorkout.name}
            </h1>
            <p className="text-xs text-gray-400 uppercase tracking-wider">
              {user.currentWorkout.focalMuscles}
            </p>
          </div>
          <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
            <Clock size={14} className="text-sparta-gold" />
            <span className="text-xs font-mono text-white">
              {user.currentWorkout.duration} min
            </span>
          </div>
        </div>
      </div>
              ...
              <div className="relative p-5">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <h3
                      className={`text-lg font-bold font-lexend ${isDone ? "text-green-400 line-through" : "text-white"}`}
                    >
                      {exercise.name}
                    </h3>
                  </div>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-full transition-all active:scale-95 ${
                      isDone
                        ...
                        : "bg-white/10 text-gray-400 hover:bg-sparta-gold hover:text-black"
                    }`}
                  >
                    <CheckCircle2
                      size={24}
                ...
                {!isDone &&
                  exercise.replacementOptions &&
                  exercise.replacementOptions.length > 0 && (
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#1e1c19] w-full max-w-sm rounded-2xl border border-sparta-gold/30 shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              <h3 className="text-white font-bold font-lexend">
                Trocar Exercício
              </h3>
              <button
            ...
            <div className="p-4 space-y-3">
              <p className="text-sm text-gray-400 mb-2">
                O aparelho está ocupado? Escolha uma alternativa para o mesmo
                grupo muscular:
              </p>
                ...
                ?.replacementOptions?.map((option) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="text-white font-bold text-sm group-hover:text-sparta-gold">
                          {option.name}
                        </h4>
                        <p className="text-xs text-gray-500 mt-1">
                          {option.sets} Séries • {option.reps} Reps
                        </p>
                      </div>
                      <RefreshCw
                        size={16}
                        className="text-gray-600 group-hover:text-sparta-gold"
                      />
                    </div>
                  </button>
                ))}
